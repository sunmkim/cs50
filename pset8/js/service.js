...
// new array to track placmark & marker
var passenger = new Array();

// declare array for passengers in bus
...
function dropoff()
{     
    // iterate through passengers currently on shuttle
    for (var i=0; i < pass_count; i++)
    {
        var d = shuttle.distance(passenger.lat, passenger.lng); 
        
        // use HOUSES array and lat & long
        var house_lat = HOUSES[i].lat;
        var house_lng = HOSUES[i].lng;
        
        // check if in range of houses
        if (d < 30)
        {
            // remove passenger from array
            bus_array.splice(i, 1);
            pass_count--;
        }
        
        // if shuttle is more than 30m away, alert user
        else if (d >= 30)
        {
            $("#announcements").html("Bus is not near a house");
        }
    }
    
    for (var j=0; j < SEATS; j++)
    {
        // let program print "empty seat" where passenger has been removed in array
        if (bus_array[j] == null)
        {
            html += "<li>Empty Seat</li>";
        }
    ...
    // if there are no passengers, return error message
    if (pass_count == 0)
    {
        $("#announcements").html("The bus is empty. No passengers to drop off.");
    }
}
...
function pickup()
{
    ...
    {  
        // detect if passenger is in range
        var d = shuttle.distance(passenger.lat, passenger.lng);
        
        // if bus is within distance and seats are available
        if (d < 15 && pass_count < SEATS)
        {
            // add passenger to bus array
            passenger[i] = bus_array[i]
            // remove placemark
            var features = earth.getFeatures();
            features.removeChild(passenger[i].placemark); 
            // remove marker
            passenger[i].marker.setMap(null);
        }
        
        // return error if bus is full
        else if (pass_count == SEATS)
        {
...
function populate()
{
    ...
    for (var i = 0; i < PASSENGERS.length; i++)
    {
        ...
        passenger[i] = PASSENGERS[i];
        passenger[i].placemark = placemark;
        passenger[i].marker = marker;
